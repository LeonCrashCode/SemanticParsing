...
class AttnDecoderRNN(nn.Module):
    def __init__(self, outer_mask_pool, rel_mask_pool, var_mask_pool, tags_info, tag_dim, input_dim, feat_dim, encoder_hidden_dim, n_layers=1, dropout_p=0.1):
        ...
        self.feat_struct = nn.Linear(self.hidden_dim + self.tag_dim, self.feat_dim)
        self.feat_tanh_struct = nn.Tanh()
	self.feat_rel = nn.Linear(self.hidden_dim + self.tag_dim, self.feat_dim)
        self.feat_tanh_rel = nn.Tanh()
	self.feat_var = nn.Linear(self.hidden_dim + self.tag_dim, self.feat_dim)
        self.feat_tanh_var = nn.Tanh()
    ...
    def forward_1(self, input, hidden, encoder_output, train, mask_variable):
        if train:
            ...
            feat_hiddens = self.feat_tanh_struct(self.feat_struct(torch.cat((attn_hiddens, embedded.transpose(0,1)), 2).view(output.size(0),-1)))
            ...
            while True:
                ...
                attn_hiddens = torch.bmm(attn_weights.unsqueeze(0), encoder_output.transpose(0, 1))
                feat_hiddens = self.feat_tanh_struct(self.feat_struct(torch.cat((attn_hiddens, embedded), 2).view(embedded.size(0),-1)))
    ...
    def forward_2(self, sentence_variable, inputs, hidden, encoder_output, least, train, mask_variable):
        ...
        if train:
            ...
            attn_weights = F.softmax(torch.bmm(output.transpose(0,1), encoder_output.transpose(0,1).transpose(1,2)).view(output.size(0),-1), 1)
            attn_hiddens = torch.bmm(attn_weights.unsqueeze(0),encoder_output.transpose(0,1))
            feat_hiddens = self.feat_tanh_rel(self.feat_rel(torch.cat((attn_hiddens, embedded.transpose(0,1)), 2).view(output.size(0),-1)))
            ...
            while True:
                ...
                attn_weights = F.softmax(torch.bmm(output, encoder_output.transpose(0,1).transpose(1,2)).view(output.size(0), -1), 1)
                attn_hiddens = torch.bmm(attn_weights.unsqueeze(0), encoder_output.transpose(0, 1))
                feat_hiddens = self.feat_tanh_rel(self.feat_rel(torch.cat((attn_hiddens, embedded), 2).view(embedded.size(0),-1)))
    ...
    def forward_3(self, inputs, hidden, encoder_output, train, mask_variable):
        if train:
            ...
            attn_weights = F.softmax(torch.bmm(output.transpose(0,1), encoder_output.transpose(0,1).transpose(1,2)).view(output.size(0),-1), 1)
            attn_hiddens = torch.bmm(attn_weights.unsqueeze(0),encoder_output.transpose(0,1))
            feat_hiddens = self.feat_tanh_var(self.feat_var(torch.cat((attn_hiddens, embedded.transpose(0,1)), 2).view(output.size(0),-1)))
            ...
            while True:
                ...
                attn_weights = F.softmax(torch.bmm(output, encoder_output.transpose(0,1).transpose(1,2)).view(output.size(0), -1), 1)
                attn_hiddens = torch.bmm(attn_weights.unsqueeze(0), encoder_output.transpose(0, 1))
                feat_hiddens = self.feat_tanh_var(self.feat_var(torch.cat((attn_hiddens, embedded), 2).view(embedded.size(0),-1)))
...
#==================================
    ...
    for instance in dev_instances:
        ...
        for i in range(len(instance[3])):
            ...
            if idx == 5 or idx == 6:
                ...
                if len(instance[4][p]) == 0:
                    ...
                    dev_input2_variable.append(Variable(torch.LongTensor([x[1] for x in instance[4][p]]), volatile=True).cuda(device))
                else:
        ...
        if use_cuda:
            ...
            dev_mask3_variables.append(Variable(torch.FloatTensor(mask3), volatile=True).cuda(device))
        else:
