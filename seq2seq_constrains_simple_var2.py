...
class AttnDecoderRNN(nn.Module):
    def __init__(self, mask_pool, tags_info, tag_dim, input_dim, feat_dim, encoder_hidden_dim, var_hidden_dim, n_layers=1, dropout_p=0.1):
        ...
        self.var_selective_matrix = Variable(torch.randn(1, self.hidden_dim, self.tag_dim))
        if use_cuda:
            self.var_selective_matrix = self.var_selective_matrix.cuda(device)

        #self.var_lstm = nn.LSTM(self.tag_dim, self.tag_dim, num_layers=1)

    def initVarHidden(self):
        if use_cuda:
            return Variable(torch.zeros(1, 1, self.var_hidden_dim)).cuda(device)
        else:
            return Variable(torch.zeros(1, 1, self.var_hidden_dim))

    def forward(self, sentence_variable, var_variable, input, hidden, encoder_output, train=True, mask_variable=None):
        ...
        if train:
            ...
            var_selective_score = torch.bmm(torch.bmm(output.transpose(0,1), self.var_selective_matrix), var_variable).view(output.size(0), -1)
            ...
            while True:
                ...
                var_selective_score = torch.bmm(torch.bmm(output, self.var_selective_matrix), var_variable).view(output.size(0), -1)
...
def train(sentence_variable, target_variable, gold_variable, mask_variable, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, back_prop=True):
    ...
    var_variable = decoder.tag_embeds(var_index).transpose(0,1).unsqueeze(0)
