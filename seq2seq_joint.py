...
def trainIters(trn_instances, dev_instances, tst_instances, encoder, decoder, print_every=100, evaluate_every=1000, learning_rate=0.001):
    ...
    for instance in trn_instances:
        ...
        input2_variables.append([])
        p = 0
        all_relations = []
        for i in range(len(instance[3])):
            idx = instance[3][i]
            if idx == 5 or idx == 6:
                all_relations = all_relation + instance[4][p]
                all_relations.append(1)
                if len(instance[4][p]) == 0:
                    input2_variables.append(None)
                elif use_cuda:
                    input2_variables[-1].append(Variable(torch.LongTensor(instance[4][p])).cuda(device))
                else:
                    input2_variables[-1].append(Variable(torch.LongTensor(instance[4][p])))
                p += 1
        if use_cuda:
            gold2_variables.append(Variable(torch.LongTensor(all_relations)).cuda(device))
        else:
            gold2_variables.append(Variable(torch.LongTensor(all_relations)))
        assert p == len(instance[4])

        input3_variables.append([])
        p = 0
        all_variables = []
        for i in range(len(instance[4])):
            idx = instance[4][i]
            if (idx >= 13 and idx < decoder.tags_info.k_rel_start) or idx >= decoder.tags_info.tag_size:
                all_variables = all_variables + instance[5][p]
                all_variables.append(1)
                if use_cuda:
                    input3_variables[-1].append(Variable(torch.LongTensor(instance[5][p])).cuda(device))
                else:
                    input3_variables[-1].append(Variable(torch.LongTensor(instance[5][p])))
        if use_cuda:
        ...
        assert p == len(instance[5])


        ##### mask1
        decoder.outer_mask_pool.reset(len(instance[0]))
        mask1 = []
        mask1.append(decoder.outer_mask_pool.get_step_mask((-2, item)))
        for item in instance[3]:
            decoder.outer_mask_pool.update((-2, item))
            mask1.append(decoder.outer_mask_pool.get_step_mask((-2, item)))
        ...
        for i in range(len(instance[3])):
            ...
            if idx == 5 or idx == 6:
                ...
                if idx == 5 or (idx == 6 and instance[3][i+1] == 4):
                    least = True
                mask2.append(decoder.rel_mask_pool.get_all_mask(len(instance[4][p]), least))
                p += 1
        assert p == len(instance[4])

        #### mask3
        decoder.var_mask_pool.reset(len(instance[0]))
        mask3 = []
        i = 0
        p = 0
