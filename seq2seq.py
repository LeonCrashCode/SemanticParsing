...
class AttnDecoderRNN(nn.Module):
    def __init__(self, tag_size, tag_dim, input_dim, att_dim, encoder_hidden_dim, n_layers=1, dropout_p=0.1):
    ...
    def forward(self, input, hidden, encoder_output, train=True):
        ...
        embedded = self.tag_embeds(input).unsqueeze(0)
...
def train(sentence_variable, target_variable, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion):
    ...
    if True:
        ...
        for di in range(target_length):
            decoder_output, decoder_hidden = decoder(decoder_input, decoder_hidden, encoder_output, train=True)
...
attn_decoder = AttnDecoderRNN(len(tag_to_ix), TAG_DIM, DECODER_INPUT_DIM, ENCODER_HIDDEN_DIM, ATTENTION_HIDDEN_DIM, n_layers=1, dropout_p=0.1)


###########################################################
# prepare training instance
trn_instances = data2instance(trn_data, [(word_to_ix,0), (pretrain_to_ix,0), (lemma_to_ix,0), (tag_to_ix, 0)])
print "trn size: " + str(len(trn_instances))
###########################################################
# prepare development instance
dev_instances = data2instance(dev_data, [(word_to_ix,0), (pretrain_to_ix,0), (lemma_to_ix,0), (tag_to_ix, 0)])
