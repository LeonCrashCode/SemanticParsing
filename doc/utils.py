...
def data2instance_structure_relation_variable3(trn_data, ixes):
	instances = []
	for one in trn_data:
		instances.append([])
		## words
		...
		for item in one[3]:
			...
			if type == -2:
				...
				for i in range(len(one[2])):
					if one[2][i] == item[:-1]:
						types.append(i)
				idx = instances[-1][2].tolist()[types[0]] + ixes[3].tag_size
				assert len(types) != 0 and idx != -1, "unrecogized local relation"
				stack.append([idx, types, -1])
